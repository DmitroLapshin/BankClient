...
public class Bank
{
    ...
    public List<UserInformation> GetAllUserInformation(UserInformation inputUser)
    {
    ...
    public void ChangeUserInformation(UserInformation inputUser, Guid id, HttpResponse response)
    {
        if (inputUser != null)
        {
            var userInformation = Users.Find(x => x.Id == id);
            ...
            response.WriteAsJsonAsync(userInformation);
        }
    ...
    /// </summary>
    /// <returns>Nothing</returns>
    public void DeleteUserInformation(Guid id, HttpResponse response)
    {
        if (string.IsNullOrEmpty(id.ToString()) == false)
        {
            var user = Users.Find(x => id == x.Id);
            Users.Remove(user);
        }
